...
public:
    ...
    enum METHOD //报文的请求方法 本项目只用到了get和post
    {
        GET = 0,
        POST,
        HEAD,
    ...
    enum CHECK_STATE//主状态机的状态
    {
        ...
        CHECK_STATE_CONTENT//解析消息体 仅限于POST请求
    };
    enum HTTP_CODE//报文解析的结果
    {
        NO_REQUEST,//请求不完整 需要继续读取请求报文数据
        GET_REQUEST,//获得了完整的HTTP请求
        BAD_REQUEST,//HTTP请求报文语法有误
        NO_RESOURCE,
        FORBIDDEN_REQUEST,
        FILE_REQUEST,
...
private:
    void init();
    HTTP_CODE process_read();//从读缓存 m_read_buf读取 并处理请求报文
    bool process_write(HTTP_CODE ret);//向写缓存m_write_buf写入响应报文数据
    HTTP_CODE parse_request_line(char *text);//主状态机解析报文中的请求行数据
    HTTP_CODE parse_headers(char *text);//主状态机解析报文中的请求头部
    ...
    void unmap();
    //根据响应报文的格式 生成对应8个部分 以下函数由do_request调用
...
public:
    static int m_epollfd;//定义指示epoll内核事件表的文件描述符
...
private:
    ...
    sockaddr_in m_address;//套接字绑定的地址
    ...
    char *m_file_address;//读取服务器上的文件的地址
    struct stat m_file_stat;
    struct iovec m_iv[2];//io向量机制iovec
    int m_iv_count;
    int cgi;        //是否启用的POST
    char *m_string; //存储请求头数据
    int bytes_to_send;//剩余发送的子节数
    int bytes_have_send;//已发送的子节数
    char *doc_root;
